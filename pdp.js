...
// El footer del PDP se agrega igual que en el PLP: botón "+" que se convierte en un
// stepper "− cantidad +" una vez agregado (en vez de un botón "Agregar" fijo).
function pdpFooterCtaHtml() {
  const product = PRODUCTS[pdpProductId];
  const qty = plpQty[pdpProductId] || 0;
  if (qty <= 0) {
    return `<button type="button" class="pdp-footer-btn" onclick="pdpAddToCart(this)">Agregar</button>`;
  }
  const leftIcon = qty === 1
    ? '<span class="msi" aria-hidden="true" style="font-size:20px">delete</span>'
    : '<span class="msi msi-fill" aria-hidden="true" style="font-size:20px">remove</span>';
  return `
    <div class="pdp-footer-stepper">
      <button type="button" onclick="pdpChangeQty(-1)" aria-label="${qty === 1 ? 'Quitar del carrito' : 'Disminuir'}">${leftIcon}</button>
      <span>${qty} ${unitLabel(product)}</span>
      <button type="button" onclick="pdpChangeQty(1)" aria-label="Aumentar"><span class="msi msi-fill" aria-hidden="true" style="font-size:20px">add</span></button>
    </div>
  `;
}

function renderPdpFooterCta() {
  const wrap = document.getElementById('pdpFooterCta');
  if (!wrap) return;
  wrap.innerHTML = pdpFooterCtaHtml();
  const product = PRODUCTS[pdpProductId];
  const qty = plpQty[pdpProductId] || 0;
  const total = qty > 0 ? parsePrice(product.price) * qty : parsePrice(product.price);
  document.getElementById('pdpFooterTotal').textContent = '$' + total.toFixed(2);
}
...
function pdpChangeQty(delta) {
  const product = PRODUCTS[pdpProductId];
  changeQty(pdpProductId, delta);
  if (isSubstituteEligible(product)) {
    if ((plpQty[pdpProductId] || 0) > 0) armSubstituteModal(pdpProductId);
    else cancelSubstituteModal(pdpProductId);
  }
  renderPdpFooterCta();
}
...
document.addEventListener('DOMContentLoaded', () => {
  renderPdpProduct();
  renderPdpNotice();
  document.getElementById('pdpSubNotice')?.addEventListener('click', () => {
